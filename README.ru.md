...
Установщик сверяет контрольную сумму до того, как файл попадёт в `PATH`, уважает `HTTPS_PROXY` и ставит в `/usr/local/bin` — или в `~/.local/bin`, если вы не root. Понимает `--version`, `--dir`, `--proxy`, `--node`, `--dry-run` и `--uninstall`.

На сервере обычно не один Node, а `sudo` видит не тот `PATH`, что вы: старый `/usr/local/bin/node` рядом со свежим в `nvm` — самая частая картина. Установщик ищет за пределами `PATH` и фиксирует найденный интерпретатор, поэтому команда работает и под `sudo`; выбрать вручную — `--node /путь/к/node`.
...
Апгрейд с 13.x — это часы работы, и смысл `--detach` в том, чтобы не сидеть у
терминала. Telegram нужен chat id, а доставать его руками — это `curl`, `grep`
по JSON и токен бота в командной строке, где его увидит `ps`. Токен уже в
конфиге, поэтому инструмент спрашивает Telegram сам:

```bash
sudo gitlab-upgrade config set telegram-token <токен>
gitlab-upgrade notify chat          # нажмите Start в чате с ботом — id придёт туда же
gitlab-upgrade notify chat --yes    # и запишется в конфиг
```

Ответ уходит в сам чат: id виден там, где его и будут искать. Если боту писали
несколько человек, команда не выбирает за вас — показывает все и ждёт решения.
Уведомления идут через тот же прокси, что и репозиторий.
