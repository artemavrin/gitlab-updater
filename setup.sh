...
gitlab_upgrade_setup() {
  ...
  while [ $# -gt 0 ]; do
    case "$1" in
      ...
      --node)    NODE="${2:?--node требует путь к интерпретатору}"; NODE_EXPLICIT=1; shift 2 ;;
      --dry-run) DRY=1; shift ;;
  ...
  need curl
  choose_node
  # До скачивания и установки: отказ после «установлено» ничего не защищает.
  check_user_owned_node
...
say() { printf '  %s\n' "$*"; }
err() { printf 'ошибка: %s\n' "$*" >&2; }
# Предупреждение — не ошибка, и префикс у него обязан быть свой. С «ошибка:»
# на непадающем сообщении читатель перестаёт различать оба: ровно так и вышло
# на живой машине — предупреждение про чужой Node прочли как шум и пошли
# дальше, потому что строкой выше стояло «установлено».
note() { printf 'внимание: %s\n' "$*" >&2; }
...
# Если root запускает интерпретатор из чужого домашнего каталога, то любой, кто
# может писать по этому пути, получает root. И это не мимолётное состояние:
# установщик вписывает интерпретатор в обёртку, то есть так будет при каждом
# запуске, всегда.
#
# Раньше здесь было предупреждение, и оно не работало. На живой машине его
# напечатали строкой ниже «установлено», с префиксом «ошибка:» — как у
# настоящих отказов, — человек прочёл это как шум и пошёл выполнять следующую
# команду. Предупреждение, которое печатают после успеха, ничего не защищает.
#
# Поэтому отказ. Осознанный выбор остаётся возможным: `--node /путь` говорит,
# что человек этот путь знает и берёт на себя, и тогда мы только предупреждаем.
check_user_owned_node() {
  ...
  if [ "$NODE_EXPLICIT" = 1 ]; then
    note "$NODE принадлежит не root (uid $owner) — указан через --node, ставлю"
    note "  кто может переписать этот файл, получит root при каждом запуске"
    return 0
  fi

  err "$NODE принадлежит не root (uid $owner), а запускать его будет root"
  err "  кто может переписать этот файл, получит root — и при каждом запуске"
  err "  поставьте Node системно и повторите:"
  err "    https://nodejs.org/dist/latest-v20.x/ — файл node-v20.*-linux-x64.tar.xz"
  err "    sudo tar -xJf node-v20.*-linux-x64.tar.xz -C /usr/local --strip-components=1"
  err "  либо возьмите этот на себя явно: setup.sh --node $NODE"
  exit 1
}
