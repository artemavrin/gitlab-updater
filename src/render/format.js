...
/**
 * Ячейка колонки: всегда шире содержимого хотя бы на пробел.
 *
 * `pad` отдаёт строку как есть, когда она длиннее колонки, — и соседняя
 * колонка приклеивается вплотную. На боевом экране это выглядело как
 * «предзагрузка пакетовидёт»: одно слипшееся слово вместо двух колонок.
 * Колонка может быть уже подписи, но разделитель обязателен всегда.
 */
export const padCell = (s, n) => pad(s, Math.max(n, [...String(s)].length + 1));
