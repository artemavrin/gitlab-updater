...
export async function commandCheck({ exec, t, flags, data, osPath, confPath = null }) {
  ...
  if (!os) return { code: EXIT.ERROR, lines: [t('error.noOs')] };
  if (!os.supported) return { code: EXIT.ERROR, lines: [t('error.unsupportedOs', { os: os.pretty })] };

  const gitlab = await detectGitlab(exec);
  if (!gitlab && !flags.from) return { code: EXIT.ERROR, lines: [t('error.noGitlab')] };

  const pkg = gitlab?.package ?? 'gitlab-ee';
  const { versions, error } = await availableVersions(exec, pkg, { confPath });
  if (!versions.length) {
    return { code: EXIT.ERROR, lines: [t('error.noPackages'), error ?? '', t('error.noPackagesHint')].filter(Boolean) };
  }
  ...
  if (!plan.target) {
    return { code: EXIT.CURRENT, lines: [t('check.current', { version: shortVersion(plan.current) })], plan };
  }
